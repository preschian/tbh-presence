...
class holds a static `StageCache` (`bgev`) for the currently loaded stage, and
...
## Field offsets (Il2CppDumper, game build 1.02.01)

Object instance fields begin at `+0x10` (klass ptr `+0x0`, monitor `+0x8`).
`CommonSaveData` gained `lastClearedStageKey` (+4 from `currentStageKey` onward).
...
at `+0x70`). `EStageType` now includes `PLAGUE` and `CONTAMINACTBOSS`.

```
PlayerSaveData.commonSaveData     +0x10
PlayerSaveData.heroSaveDatas      +0x68   (List<HeroSaveData>: items +0x10, count +0x18; element ptrs from items+0x20)
...
CommonSaveData.lastClearedStageKey +0x60  (int; inserted @1.02.01)
CommonSaveData.currentStageKey    +0x64   (int)
CommonSaveData.currentStageWave   +0x68   (int)
...
StageInfoData.STAGETYPE           +0x40   (EStageType: 0 NORMAL, 1 ACTBOSS, 2 PLAGUE, 3 CONTAMINACTBOSS)
...
we.StageCache.StageInfoData       +0x10   (bgez)
Il2CppClass.static_fields         +0xB8
we.vy static block -> StageCache  +0xA8   (bgev)
```
...
- `GameInterop.cs` — signature-based access to obfuscated members (including
  box counts keyed by `EBoxType` + `EContentType` since 1.02.01).
